...
function regionTag(name,uid) {
	...
	if( uid ) {
		...
		str = [ "<div class='region-tag' id='" + uid + "' style='padding:3px'>",
	...
	*/
	...
	for( var sl in ImageInfo ) {
		...
		for( var i = 0; i < slice.Regions.length; i++ )
		{
			...
			//el.contour=JSON.stringify(contour);
			el.contour = contour;
		...
		console.log("< start post contours information", value);
		$.ajax({
			type: 'POST',
			url: '/uploadinfo',
			data: fd,
			...
			contentType: false,
			success: function() {
				console.log("< upload contours success");
			},
			error: function() {
				console.log("< upload contours failed");
			}
		});
		...
		//
		// if( debug ) console.log("Bofore upload");
		// // post data to database
		// (function(sl, h) {
		// 	// console.log('saving slice ', sl);
		// 	console.log('upload contour and conclusion');
		// 	$.ajax({
		// 		type: 'POST',
		// 		url:'/uploadinfo',
		// 		data:{
...
function save() {
	...
	for( i = 0; i < ImageInfo[currentImage]["Regions"].length; i++ )
	{
		...
		console.log(ImageInfo[currentImage]["Regions"][i].path.segments.length);
...
//    console.log(ImageInfo[currentImage]["projectID"]);
